...
void ApplicationFunctionSet::ApplicationFunctionSet_Follow(void)
{
  ...
  {
    Serial.println("IN FOLLOW MODE");  // add this
    ...
    Serial.print("DISTANCE: ");         // add this
    Serial.println(ULTRASONIC_Get);     // add this


    if (ULTRASONIC_Get == 0)
    {
      // sensor out of range — stop
      ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
    }
    else if (function_xxx(ULTRASONIC_Get, 0, 15))
    {
      // too close — back up
      ApplicationFunctionSet_SmartRobotCarMotionControl(Backward, 120);
    }
    else if (function_xxx(ULTRASONIC_Get, 15, 25))
    {
      // perfect distance — hold still
      ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
    }
    else if (function_xxx(ULTRASONIC_Get, 25, 60))
    {
...
void ApplicationFunctionSet::ApplicationFunctionSet_RoamMode(void)
{
  static unsigned long segment_timer = 0;
  static unsigned long mode_timer = 0;   // ADD THIS
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == Follow_mode)
  {
    if (first_is == true)
    {
      ...
      mode_timer = millis();             // ADD THIS
      first_is = false;
    }

    // STOP after 15 seconds total — tune this number
    if (millis() - mode_timer > 10000)
{
  // blast forward at full speed for 2 seconds — very obvious escape
